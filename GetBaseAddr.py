# -*- coding:utf-8 -*-
"""
@author: Source: https://wrpzkb.cn/pythonGetBaseAddr/#6-%E5%AE%8C%E6%95%B4%E4%BB%A3%E7%A0%81 : Modified by: lejara
@file: GetBaseAddr.py
...
# Note: Original code had this, did not work for me. kept getting error 87 
# class MODULEENTRY32(Structure):
...
def _GetProcessId(className,windowName):
    hGameWindow = FindWindow(className, None)
...
def GetRetroArc_DLL_Address():

    ProcessId = _GetProcessId("retroarch", None)
    _hGameHandle = _GetPorcessHandle(ProcessId)
    moduleName = GetProcessImageBase(ProcessId, "genesis_plus_gx_libretro.dll")
    c_long_p = ctypes.POINTER(ctypes.c_ulong)
    addre = ctypes.cast(ctypes.addressof(moduleName), c_long_p).contents
